...
<!--      class="gather radionList"-->
      ...
      <div :class="[item.controlType+'_box',{'addColor':report.value}]">
        <!--<mt-radio
        ...
        <input type="text" class="Extend_input" v-if="report.value == '其他' && this.item.baseProperty.controlIsExtend"
               ...
               placeholder="请输入"/>-->
        {{report.value || '(空)'}}
        <span :class="[{'addColor':report.value2}]" v-if="item.baseProperty.controlIsExtend && report.value == '其他' && report.value2">{{ `(${report.value2})` }}</span>
      </div>
